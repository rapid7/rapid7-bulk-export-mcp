        ...
        try:
            if export_type == "vulnerability":
                ...
                for i, data in enumerate(file_data):
                    ...
                    if len(data) < 100:
                        validation_warnings.append(f"File {i+1}: unusually small ({len(data)} bytes)")
                ...
                if row_count == 0 and len(file_data) > 0:
                    validation_warnings.append(
                        "⚠️  All files downloaded successfully but 0 rows loaded. "
                        "This may indicate a schema mismatch or empty export from the API."
                    )
